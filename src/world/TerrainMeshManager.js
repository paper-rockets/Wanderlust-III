...
export class TerrainMeshManager {
    ...
    update(playerX, playerZ, animeWaterSystem) {
        ...
        // 1. Single-pass height & position calculation via direct TypedArray access
        const posArr = pos.array;
        for (let i = 0; i < pos.count; i++) {
            const i3 = i * 3;
            const localX = posArr[i3];
            const localZ = posArr[i3 + 2];
        ...
        // 2. Direct grid normal calculation from buffer array
        ...
        for (let r = 0; r < N; r++) {
            ...
            for (let c = 0; c < N; c++) {
                const idx = rOffset + c;
                const idx3 = idx * 3;
                const hL = posArr[(rOffset + Math.max(0, c - 1)) * 3 + 1];
                const hR = posArr[(rOffset + Math.min(N - 1, c + 1)) * 3 + 1];
                const hD = posArr[(rPrev + c) * 3 + 1];
                const hU = posArr[(rNext + c) * 3 + 1];
                const dx = hL - hR;
                const dz = hD - hU;
        ...
        // 3. Direct vertex color computation into array
        ...
        for (let i = 0; i < pos.count; i++) {
            ...
            colArr[i3] = this.tempColor.r;
            colArr[i3 + 1] = this.tempColor.g;
            colArr[i3 + 2] = this.tempColor.b;
        }
